...
export default function Register() {
  ...
  // ==========================================
  // 🚀 MENANGKAP KODE REFERRAL DARI URL
  // ==========================================
  ...
  // State untuk Checkbox Ketentuan Layanan
  ...
  const handleRegister = async (e) => {
    ...
    // Keamanan tambahan: pastikan checkbox dicentang sebelum memproses
    if (!isAgreed) {
    ...
    const payload = {
        ...
        // ==========================================
        // 🚀 MENYISIPKAN KODE REFERRAL KE BACKEND
        // ==========================================
    ...
    <div className="flex min-h-[85vh] items-center justify-center p-4 bg-slate-50 dark:bg-slate-900 transition-colors duration-300">
      ...
      <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 shadow-2xl dark:border-slate-800 dark:bg-slate-950 animate-in fade-in zoom-in duration-300">
        <div className="mb-8 text-center">
          ...
          {/* ==========================================
              🚀 NOTIFIKASI KECIL JIKA MEMAKAI REFERRAL
              ========================================== */}
          {referralCode && (
              <p className="mt-3 text-xs font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 py-1.5 px-3 rounded-full inline-block border border-emerald-100 dark:border-emerald-800/30">
                  🎁 Anda diundang dengan kode: {referralCode}
              </p>
          )}
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-5">
          ...
          {/* ==========================================
              🚀 CHECKBOX KETENTUAN LAYANAN & ANTI ABUSE
              ========================================== */}
          <div className="flex items-start gap-3 py-1">
            <div className="flex h-5 items-center">
              <input
                ...
                className={`h-4 w-4 cursor-pointer rounded border-slate-300 bg-slate-50 transition-colors focus:ring-2 dark:border-slate-700 dark:bg-slate-900 ${color.ring}`}
              />
            </div>
            <label htmlFor="terms" className="text-xs leading-relaxed text-slate-600 dark:text-slate-400 cursor-pointer select-none">
              Saya menyetujui <Link to="/ketentuan" className={`font-bold hover:underline ${color.text}`}>Ketentuan Layanan</Link> dan menjamin tidak akan menggunakan layanan ini untuk aktivitas ilegal (Spam, Scam, Perjudian, dll).
            </label>
          </div>
          ...
          <button 
            ...
            className={`flex w-full items-center justify-center gap-2 rounded-xl py-4 font-bold shadow-lg transition-all active:scale-95 ${
              !isAgreed ? 'bg-slate-300 text-slate-500 cursor-not-allowed dark:bg-slate-800 dark:text-slate-500 shadow-none' : color.btn
            }`}
          >
            {loading ? (
                <>
      ...
      {/* --- TOAST NOTIFICATION --- */}
      <div className={`fixed bottom-10 left-1/2 z-[100] flex -translate-x-1/2 transform items-center gap-3 rounded-full px-6 py-3 shadow-2xl transition-all duration-300 w-max max-w-[90vw] ${toast.show ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0 pointer-events-none'} ${toast.type === 'success' ? 'bg-slate-900 text-white dark:bg-white dark:text-slate-900' : 'bg-red-500 text-white'}`}>
